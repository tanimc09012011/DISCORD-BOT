...
let admins = [
    { id: "head-admin", email: HEAD_ADMIN_EMAIL, pass: HEAD_ADMIN_PASS, name: "Head Admin", isHead: true, status: "Active", allowedSystem: "all" }
];
let adminServers = {}; // Stores server ID assigned to each admin email/id
let adminPermissions = {}; // Stores specific system permissions for sub admins
let scheduledJobs = {};
...
app.post('/api/admin/login', (req, res) => {
    ...
    if (admin) {
        return res.json({ 
            status: 'success', 
            admin: { email: admin.email, name: admin.name, isHead: admin.isHead, allowedSystem: admin.allowedSystem || 'all' },
...
app.get('/api/admin/list', verifyAdmin, (req, res) => {
    res.json(admins.map(a => ({ 
        id: a.id, 
        email: a.isHead ? 'Hidden (Head Admin)' : a.email, // Fix 6: Sub admin cannot see head admin email or other emails if required, but showing masked/safe text
        name: a.name, 
        isHead: a.isHead, 
        status: a.status,
        assignedServer: adminServers[a.email] || null,
        allowedSystem: a.allowedSystem || 'all'
    })));
});
...
app.post('/api/admin/add', verifyAdmin, (req, res) => {
    ...
    const { email, password, name, serverId, allowedSystem } = req.body;
    ...
    admins.push({ 
        ...
        isHead: false, 
        status: 'Active',
        allowedSystem: allowedSystem || 'all'
    });
    if (serverId) {
...
// Guilds & Category-wise Channels (Fix 7: Optimized and cached response to eliminate 10-12s hang)
...
// Add Tick/Cross Reaction to Message (Fix 3)
...
// Join Voice Channel (Fix 1: Two-way audio streaming & mic/speaker bridging)
app.post('/api/voice/join', async (req, res) => {
    try {
        ...
        // Listen to VC incoming speech and pipe to WebSocket clients
        activeVoiceConnection.receiver.speaking.on('start', (userId) => {
            const audioStream = activeVoiceConnection.receiver.subscribe(userId, {
                end: {
        ...
        res.json({ status: 'success', message: 'বটটি ভয়েস চ্যানেলে সফলভাবে যুক্ত হয়েছে!' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
// Voice Mute / Deafen Control Endpoints
...
// Get Voice Channel Participants (Fix)
app.get('/api/voice/participants/:channelId', async (req, res) => {
    try {
        const channel = await client.channels.fetch(req.params.channelId);
...
// WebSocket Audio Stream Handler Fixed for Two-way Voice Input/Output
