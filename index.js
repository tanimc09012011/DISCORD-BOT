...
app.get('/api/guild/:guildId/categories-channels', async (req, res) => {
    try {
        ...
        channels.forEach(ch => {
            // Text Channel (0) and Announcement/News Channel (5)
...
// Role Assign with Auto React Tick
app.post('/api/assign-role-react', async (req, res) => {
    try {
        const { guildId, userId, roleId, channelId, messageId } = req.body;
        ...
        res.json({ status: 'success', message: `${member.user.tag}-কে ${role.name} রোল আসাইন করা হয়েছে এবং মেসেজে টিকমার্ক দেয়া হয়েছে!` });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/api/schedule-message-datetime', upload.single('file'), (req, res) => {
    try {
        ...
        res.json({ status: 'success', message: `মেসেজটি ${targetDate.toLocaleString('bn-BD')} সময়ে পাঠানোর জন্য শিডিউল করা হয়েছে!` });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/api/voice/join', async (req, res) => {
    try {
        ...
        res.json({ status: 'success', message: 'বটটি সফলভাবে ভয়েস চ্যানেলে যুক্ত হয়েছে!' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
// WebSocket Audio Stream Handler (Web Mic to Discord Voice)
